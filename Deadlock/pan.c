...
short src_ln0 [] = {
	  0,  11,  12,  14,  14,  10,  17,  10, 
	 17,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "lab4_deadlock.pml", 1, 8 },
	{ "-", 9, 10 }
};
uchar reached0 [] = {
	  0,   1,   0,   0,   0,   0,   1,   1, 
...
{	int j, h = now._nr_pr;
	...
	case 1:	/* :init: */
		...
		/* params: */
		/* locals: */
		((P1 *)pptr(h))->philosophers = 5;
#ifdef VAR_RANGES
		logval(":init::philosophers", ((P1 *)pptr(h))->philosophers);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* philosopher */
		...
		/* locals: */
#ifdef VAR_RANGES
		logval("philosopher:my_id", ((P0 *)pptr(h))->my_id);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.forks[0] = addqueue(calling_pid, 1, 0);
		now.forks[1] = addqueue(calling_pid, 2, 0);
		now.forks[2] = addqueue(calling_pid, 3, 0);
		now.forks[3] = addqueue(calling_pid, 4, 0);
		now.forks[4] = addqueue(calling_pid, 5, 0);
#ifdef VAR_RANGES
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.forks[0]) )
			 ||     q_full((int) ( now.forks[1]) )
			 ||     q_full((int) ( now.forks[2]) )
			 ||     q_full((int) ( now.forks[3]) )
			 ||     q_full((int) ( now.forks[4]) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.forks[0]) )
			 ||    !q_full((int) ( now.forks[1]) )
			 ||    !q_full((int) ( now.forks[2]) )
			 ||    !q_full((int) ( now.forks[3]) )
			 ||    !q_full((int) ( now.forks[4]) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.forks[0]) )
			 ||     !q_len((int) ( now.forks[1]) )
			 ||     !q_len((int) ( now.forks[2]) )
			 ||     !q_len((int) ( now.forks[3]) )
			 ||     !q_len((int) ( now.forks[4]) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.forks[0]) )
			 ||      q_len((int) ( now.forks[1]) )
			 ||      q_len((int) ( now.forks[2]) )
			 ||      q_len((int) ( now.forks[3]) )
			 ||      q_len((int) ( now.forks[4]) )))
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  fork:	1\n");
	printf("	chan forks[0] (=%d):	len %d:\t", now.forks[0], q_len(now.forks[0]));
	c_chandump(now.forks[0]);
	printf("	chan forks[1] (=%d):	len %d:\t", now.forks[1], q_len(now.forks[1]));
	c_chandump(now.forks[1]);
	printf("	chan forks[2] (=%d):	len %d:\t", now.forks[2], q_len(now.forks[2]));
	c_chandump(now.forks[2]);
	printf("	chan forks[3] (=%d):	len %d:\t", now.forks[3], q_len(now.forks[3]));
	c_chandump(now.forks[3]);
	printf("	chan forks[4] (=%d):	len %d:\t", now.forks[4], q_len(now.forks[4]));
	c_chandump(now.forks[4]);
}
void
...
{	/* int i; */
	...
	printf("	byte   philosophers:	%d\n", ((P1 *)pptr(pid))->philosophers);
		break;
	case 0:
